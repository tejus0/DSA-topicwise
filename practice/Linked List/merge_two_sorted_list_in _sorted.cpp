...
Node *takeinput()
{
    ...
    Node *head = NULL;
    while (d != -1)
    {                          //we can also take input from a
        insertAtHead(head, d); //file even that is not having -1 at end , by while(cin>>d)
        cin >> d;
    }
    return head;
}
...
{ //head is passed by refernce as it needs to be updated when we call reverse
    Node *curr = head;
    Node *prev = NULL;
    Node *later;
    while (curr != NULL)
...
Node *reverseRec(Node *&head)
{
    ...
    { //when list is empty or last element is reached
        return head;
    }
...
{
    //check for no node or only one node
...
int main()
{
   ...
   cout<<mid->data<<endl;*/
    ...
    Node *head2;
    cin >> head1 >> head2;
    Node *final = merge(head1, head2); // if we want increasing order then we should give both list as sorted
    cout << final << endl;
