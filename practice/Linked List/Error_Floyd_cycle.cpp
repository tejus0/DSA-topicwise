...
Node* takeinput(){
    ...
    cin>>d;
    Node*head=NULL;
    while(d!=-1){  //we can also take input from a   
...
Node* mergesort(Node* head){
    ...
    a=mergesort(a);
    b=mergesort(b);
    //merge the half lists after sorting
    Node*c=merge(a,b);
    return c;
}
...
int main(){
  ...
  /* Node* mid=mid_of_list(head);
   ...
   cin>>head;
   cout<<head;
   /*Node* final= mergesort(head); // if we want increasing order then we should give both list as sorted
   cout<<final<<endl;
   */
    if (detectCycle(head)){
